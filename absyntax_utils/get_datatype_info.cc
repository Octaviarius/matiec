...
bool get_datatype_info_c::is_arraytype_equal_relaxed(symbol_c *first_type, symbol_c *second_type) {
  ...
  for (int i = 0; i < subrange_list_1->n; i++) {
     ...
     */
    if (    (subrange_1->lower_limit->const_value.int64_val.is_valid() || subrange_1->lower_limit->const_value.uint64_val.is_valid())
         && (subrange_2->lower_limit->const_value.int64_val.is_valid() || subrange_2->lower_limit->const_value.uint64_val.is_valid())
         && (subrange_1->upper_limit->const_value.int64_val.is_valid() || subrange_1->upper_limit->const_value.uint64_val.is_valid())
         && (subrange_2->upper_limit->const_value.int64_val.is_valid() || subrange_2->upper_limit->const_value.uint64_val.is_valid())
       ) {
